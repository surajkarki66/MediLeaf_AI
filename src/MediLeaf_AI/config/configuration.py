...
class ConfigurationManager:
    ...
    def get_prepare_base_model_config(self) -> PrepareBaseModelConfig:
        ...
        create_directories([Path(config.root_dir), Path(
            config.base_model_path), Path(config.updated_base_model_path)])
    ...
    def get_training_config(self) -> TrainingConfig:
        ...
        training_data = os.path.join(
            self.config.data_ingestion.unzip_dir, "dataset_v2", "train")
        validation_data = os.path.join(
            self.config.data_ingestion.unzip_dir, "dataset_v2", "val")
        create_directories([
            Path(training.root_dir),
            Path(training.trained_model_dir),
            Path(training.training_graphs_dir),
            Path(training.training_scores_dir),
        ])
        ...
        training_config = TrainingConfig(
            ...
            training_scores_dir=Path(training.training_scores_dir),
    ...
    def get_validation_config(self) -> EvaluationConfig:
        ...
        test_data = os.path.join(
            self.config.data_ingestion.unzip_dir, "dataset_v2", "test")
        create_directories([Path(testing.root_dir), Path(
            testing.testing_cm_dir), Path(testing.testing_scores_dir)])
        eval_config = EvaluationConfig(
            root_dir=Path(testing.root_dir),
            path_of_model=Path(os.path.join(
                training.root_dir, self.get_prepare_base_model_config().params_pre_trained_model)),
            test_data=Path(test_data),
            all_params=self.params,
            testing_cm_dir=Path(testing.testing_cm_dir),
            testing_scores_dir=Path(testing.testing_scores_dir),
            evaluation_metrics_dir=Path(testing.testing_cm_dir),
            params_image_size=self.params.IMAGE_SIZE,
